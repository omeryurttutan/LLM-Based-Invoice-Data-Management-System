...
export function SystemHealthPanel() {
    ...
    const resources = data.resources || {
        jvmHeapUsage: 0, jvmHeapMax: 1,
        dbActiveConnections: 0, dbMaxConnections: 1,
        diskUsage: 0, diskTotal: 1
    };
                            ...
                            <div>
                                <div className="flex justify-between text-xs mb-1">
                                    ...
                                    <span className="text-muted-foreground">
                                        {Math.round((resources.jvmHeapUsage / resources.jvmHeapMax) * 100)}%
                                        ({Math.round(resources.jvmHeapUsage / 1024 / 1024)}MB / {Math.round(resources.jvmHeapMax / 1024 / 1024)}MB)
                                    </span>
                                </div>
                                <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                                    <div
                                        className={`h-full ${(resources.jvmHeapUsage / resources.jvmHeapMax) > 0.85 ? 'bg-red-500' : 'bg-blue-500'
                                            }`}
                                        style={{ width: `${Math.min(100, (resources.jvmHeapUsage / resources.jvmHeapMax) * 100)}%` }}
                                    />
                                </div>
                            </div>
                            <div>
                                <div className="flex justify-between text-xs mb-1">
                                    ...
                                    <span className="text-muted-foreground">
                                        {resources.dbActiveConnections} / {resources.dbMaxConnections}
                                    </span>
                                </div>
                                <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                                    <div
                                        className="h-full bg-green-500"
                                        style={{ width: `${Math.min(100, (resources.dbActiveConnections / resources.dbMaxConnections) * 100)}%` }}
                                    />
                                </div>
                            </div>
                            <div>
                                <div className="flex justify-between text-xs mb-1">
                                    ...
                                    <span className="text-muted-foreground">
                                        {Math.round((resources.diskUsage / resources.diskTotal) * 100)}%
                                        ({Math.round(resources.diskUsage / 1024 / 1024 / 1024)}GB / {Math.round(resources.diskTotal / 1024 / 1024 / 1024)}GB)
                                    </span>
                                </div>
                                <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                                    <div
                                        className={`h-full ${(resources.diskUsage / resources.diskTotal) > 0.8 ? 'bg-red-500' : 'bg-yellow-500'
                                            }`}
                                        style={{ width: `${Math.min(100, (resources.diskUsage / resources.diskTotal) * 100)}%` }}
                                    />
                                </div>
                            </div>
                        </div>
