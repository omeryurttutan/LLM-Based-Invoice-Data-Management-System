    ...
    return (
      <Card className="col-span-1 h-[400px]">
        <CardHeader>
          ...
          <CardDescription>Onay bekleyen son faturalar</CardDescription>
        </CardHeader>
    ...
    <Card className="col-span-1 h-[400px] flex flex-col">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle>Bekleyen İşlemler ({data?.totalPending || 0})</CardTitle>
          <Badge variant="secondary" className="bg-yellow-100 text-yellow-800 hover:bg-yellow-100">
            {data?.totalPending} Adet
          </Badge>
        </div>
        <CardDescription>Onay bekleyen son faturalar</CardDescription>
      </CardHeader>
      <CardContent className="flex-1 overflow-auto">
        {invoices.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground p-4">
                ...
                <p className="text-sm">Tüm faturalar işlenmiş durumda.</p>
            </div>
        ) : (
            ...
            {invoices.map((invoice) => (
                ...
                <div className="space-y-1">
                    ...
                    {invoice.invoiceNumber} • {new Intl.NumberFormat('tr-TR', { style: 'currency', currency: invoice.currency }).format(invoice.totalAmount)}
                    </p>
                    {invoice.daysPending > 7 && (
                        <div className="flex items-center text-xs text-red-500 mt-1">
                            <AlertCircle className="h-3 w-3 mr-1" />
                            <span>{invoice.daysPending} gündür bekliyor</span>
                        </div>
                    )}
                </div>
                <Button asChild size="sm" variant="outline">
                    <Link href={`/invoices/${invoice.id}/verify`}>Doğrula</Link>
                </Button>
                </div>
            ))}
            </div>
