...
export const VerificationForm: React.FC<VerificationFormProps> = ({ invoice }) => {
    ...
    const handleSaveDraft = useCallback(async () => {
        ...
        try {
            ...
            toast.success(t('messages.draftSaved'), { description: t('messages.draftSavedDesc') });
        } catch (error) {
            console.error("Taslak kaydedilemedi", error);
            toast.error(t('messages.draftError'), { description: t('messages.draftErrorDesc') });
        } finally {
    ...
    const handleRevalidate = async () => {
        ...
        try {
            ...
            toast.success(t('messages.revalidateSuccess'), { description: t('messages.revalidateSuccessDesc', { score: result.confidenceScore }) });
        } catch (error) {
            toast.error(t('messages.revalidateError'), { description: t('messages.revalidateErrorDesc') });
        } finally {
    ...
    const onVerify = async (data: VerificationFormData) => {
        ...
        try {
            ...
            toast.success(t('messages.verifySuccess'), { description: t('messages.verifySuccessDesc') });

            router.push('/invoices'); // Or next invoice
        } catch (error) {
            toast.error(t('messages.verifyError'), { description: t('messages.verifyErrorDesc') });
        } finally {
            setIsSubmitting(false);
    ...
    const handleReject = async () => {
        if (!rejectReason) {
            toast.error(t('messages.rejectError'), { description: t('messages.reasonRequired') });
            return;
        }
        ...
        try {
            ...
            toast.success(t('messages.rejectSuccess'), { description: t('messages.rejectSuccessDesc') });
            router.push('/invoices');
        } catch (error) {
            toast.error(t('messages.rejectError'), { description: t('messages.rejectErrorDesc') });
        } finally {
            setIsSubmitting(false);
            ...
            <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900 border-l border-gray-200 dark:border-gray-800 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-6">
                    <form onSubmit={handleSubmit(onVerify)} className="space-y-6">
                        ...
                        {/* Section A: General */}
                        <FormSection title={t('form.general')}>
                            ...
                            <ValidatedField issue={getIssue('invoiceDate')}>
                                <Label>{t('form.invoiceDate')}</Label>
                            ...
                            <ValidatedField issue={getIssue('currency')}>
                                ...
                                >
                                    <SelectTrigger>
                                        <SelectValue placeholder={t('form.selectPlaceholder') || "Seçiniz"} />
                                    </SelectTrigger>
                                    <SelectContent>
                        ...
                        {/* Section B: Supplier */}
                        <FormSection title={t('form.supplier')}>
                            <ValidatedField issue={getIssue('supplierName')}>
                                <Label>{t('form.supplierName')}</Label>
                        ...
                        {/* Section E: Amounts */}
                        <FormSection title={t('form.amounts')}>
                        ...
                        <FormSection title={t('form.notes')}>
                            <div className="col-span-1 md:col-span-2">
                                <Textarea {...methods.register('notes')} disabled={isReadOnly} placeholder={t('form.notesPlaceholder')} />
                            </div>
                        </FormSection>
                    ...
                    <div className="p-4 bg-white dark:bg-gray-950 border-t border-gray-200 dark:border-gray-800 flex justify-between items-center gap-4">
                        ...
                        <div className="flex gap-2">
                            <Dialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
                                ...
                                <DialogContent>
                                    <DialogHeader>
                                        <DialogTitle>{t('dialogs.reject.title')}</DialogTitle>
                                        <DialogDescription>
                                    ...
                                    <Textarea
                                        ...
                                        placeholder={t('dialogs.reject.placeholder')}
                                        rows={3}
                                    />
                                    <DialogFooter>
                                        <Button variant="ghost" onClick={() => setShowRejectDialog(false)}>{t('dialogs.reject.cancel') || 'İptal'}</Button>
                                        <Button variant="destructive" onClick={handleReject} disabled={isSubmitting}>{t('dialogs.reject.confirm')}</Button>
                                    </DialogFooter>
                                </DialogContent>
                            </Dialog>
                            ...
                            <Dialog open={showVerifyDialog} onOpenChange={setShowVerifyDialog}>
                                <DialogTrigger asChild>
                                    <Button className="bg-green-600 hover:bg-green-700 text-white" disabled={isSubmitting}>
                                        ...
                                        {t('dialogs.verify.confirm') + ' ' + (t('saveDraft').toLowerCase().includes('kaydet') ? '& Kaydet' : 'and Save')}
                                    </Button>
                                </DialogTrigger>
                                <DialogContent>
                                    <DialogHeader>
                                        <DialogTitle>{t('dialogs.verify.title')}</DialogTitle>
                                        <DialogDescription>
                                            {t('dialogs.verify.description')}
                                        </DialogDescription>
                                    </DialogHeader>
                                    <DialogFooter>
                                        <Button variant="ghost" onClick={() => setShowVerifyDialog(false)}>{t('dialogs.reject.cancel') || 'İptal'}</Button>
                ...
                <div className="px-4 py-1 bg-gray-100 dark:bg-gray-800 text-[10px] text-gray-500 flex justify-center gap-4 border-t border-gray-200 dark:border-gray-700">
                    <span>{t('shortcuts')} <strong>{t('tabShortcut')}</strong></span>
                    <span><strong>{t('ctrlEnterShortcut')}</strong></span>
                    <span><strong>{t('ctrlSShortcut')}</strong></span>
                </div>
