"""
Unit tests for GeminiProvider — Phase 15.
...
class TestGeminiProvider:
    ...
    def test_provider_name(self, provider):
        assert provider.provider_name == LLMProviderNames.GEMINI_3_FLASH
        assert provider.provider_name == "GEMINI_3_FLASH"
    ...
    async def test_generate_sends_image_as_inline_data(self, mock_model_cls, provider):
        ...
        # Verify the content array was passed
    ...
    async def test_generate_uses_system_instruction(self, mock_model_cls, provider):
        ...
        # Verify GenerativeModel was created with system_instruction
    ...
    async def test_generate_rate_limit_error(self, mock_model_cls, provider):
        """ResourceExhausted → LLMRateLimitError (after retries)."""
        mock_model = MagicMock()
        mock_model.generate_content_async = AsyncMock(
    ...
    async def test_generate_server_error(self, mock_model_cls, provider):
        """InternalServerError → LLMServerError (after retries)."""
        mock_model = MagicMock()
        mock_model.generate_content_async = AsyncMock(
    ...
    @pytest.mark.asyncio
    @patch("app.services.llm.providers.gemini_provider.genai.GenerativeModel")
    async def test_generate_retries_on_server_error(self, mock_model_cls, provider):
        """Server errors trigger retries before raising."""
        mock_response = MagicMock()
        ...
        # Every call to GenerativeModel() returns the same mock instance
        mock_model_cls.return_value = mock_model
